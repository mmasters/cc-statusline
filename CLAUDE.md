...
A custom Claude Code statusline — a single bash script (`statusline.sh`) that transforms Claude Code's JSON session data into a 6-line colored terminal display showing hostname, directory/git info, environment, context usage, session timer, cost/tokens, and rotating tips.
